...
class CalculationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(isset($request->sub_po)){
        $sub_po=$request->sub_po;
        }else{
        $sub_po=null;
        }
        ...
        $lastname=null;
         ...
         if(isset($request->uuid)){
            ...
            if($pev_data){
             $firstname=$pev_data->firstname;
             $lastname=$pev_data->lastname;
             }
            ...
            if(isset($request->restore)){
                ...
                if($pev_data){
                            ...
                            /* history value available */

                            if($pev_data->single!=-1 || $pev_data->day!=-1 || $pev_data->night!=-1 || $pev_data->excl_night!=-1){
                                        ...
                                        if($pev_data->meter_type){
                                            ...
                                            if($registerDay!=-1 && $registerNight!=-1 && $registerExclNight==-1 ){
                                                $Metertype='double';
                                            }elseif($registerNormal!=-1 && $registerDay==-1 && $registerNight==-1 && $registerExclNight==-1){
                                                $Metertype='single';
                                            }elseif($registerNormal!=-1 && $registerDay==-1 && $registerNight==-1 && $registerExclNight!=-1){
                        ...
                        if($pev_data->gas_consumption!=-1){
                            ...
                            $registerExclNight=-1;
                            $first_residence=1;
                            $decentralise_production=null;
                            $capacity_decentalise=null;
                            $registerG=25000; 
                            $Metertype='single';
                            $IncludeG=1;
                            $IncludeE=1;
            ...
            /*default-input-section-start-with category */
      ...
      if(isset($request->checkup)){
        ...
        if($sumRegisters==-1 && $registerG==-1){ 
        $status="false";
        return $status;
        }
        
        /*end of parameters and values for calculation*/
    ...
    if($sumRegisters>=0 && $registerG>=0){
        ...
        foreach($result['products'] as $getProducts){
                        ...
                        $products['parameters']['values']=[
                            ...
                            'estimate_cunsomption' => $request->estimate_cunsomption,
        ...
        if($sumRegisters>=0 && $registerG==-1){
            ...
            if($locale=='nl'){
                ...
                if(isset($getProducts->staticData->info_NL)){ $description=iconv("cp1252", "utf-8//TRANSLIT",$getProducts->staticData->info_NL); }
                ...
                $product_popularity_count=SupplierPopularity::where('product_id',$getProducts->product_id)->get();
     ...
     if(isset($getProducts->fixed_indexed)){ 
        ...
        if($priceType=='Fix'){
            ...
            $pt='variable';
               ...
               $product_popularity_count=SupplierPopularity::where('product_id',$getProducts->product_id)->where('customer_group',$customerGroup)->where('comparison_type',$comparisonType)->get();
               $product_popularity_total=SupplierPopularity::where('customer_group',$customerGroup)->where('comparison_type',$comparisonType)->get();
                $total=0;
               foreach($product_popularity_total as $product_popularity_total){
        ...
        if($getProducts->staticGasDetails->FF_pro_rata==""){
         ...
         if($locale=='nl'){
                ...
                if($comparisonType=='pack'){
                 ...
                 $supplier=Supplier::where('commercial_name',$supplier)->first();
                    ...
                    $products['supplier'] = [
                        ...
                        'logo' => utf8_encode($supplier->logo_large),
                 ...
                 // price-breakdown-electricity               
                    ...
                    if($sumRegisters>=0 && $registerG==-1){
                        ...
                        $fixedFee=$getProducts->ff_day_night+$getProducts->ff_excl_night;
                        }
                        ...
                        $cert=str_replace(',', '', number_format($certificate*100, 1, '.', ','));
                          ...
                          $tax=(($result['electricity']['tax']->energy_contribution+$result['electricity']['tax']->federal_contribution+$result['electricity']['tax']->connection_fee)*$sumRegisters)/100+$result['electricity']['tax']->contribution_public_services+$fixed_tax;
                            ...
                            $total=$totalEnet=(float)($fixedFee)+$certificate*100+($getProducts->price_single*$registerNormal)+($getProducts->price_day*$registerDay)+($getProducts->price_night*$registerNight)+($getProducts->price_excl_night*$registerExclNight);
                           
                            
                            if($sumRegisters>0){
                            $energyCost=($total/$sumRegisters)*100;
                            $eCost=((($getProducts->price_single*$registerNormal+$getProducts->price_dayE*$registerDay+$getProducts->price_night*$registerNight+$getProducts->price_excl_night*$registerExclNight)+$fixedFee+$cert)/$sumRegisters); 
                            ...
                            'energy_cost' =>str_replace(',', '', number_format(($eCost), 4, '.', ',')), //$energyCost,
                            'total' =>$total/10000 ,
                    ...
                    $excl_night=$getProducts->price_excl_night*$registerExclNight;
                        ...
                        $products['price']['breakdown']['electricity']['energy_cost'] = [
                           ...
                           if($FirstResidence==true){
                            ...
                            $total=$totalEnet=$fixedFee+$certificate+($getProducts->price_singleE*$registerNormal+$getProducts->price_dayE*$registerDay+$getProducts->price_nightE*$registerNight+$getProducts->price_excl_nightE*$registerExclNight)/100;
                            if($sumRegisters>0){
                                $totSum=$total/$sumRegisters;
                            }else{
                            ...
                            $products['price']['breakdown']['electricity']['unit_cost'] = [
                            'energy_cost' =>str_replace(',', '', number_format(($totSum), 5, '.', ',')) ,
                            'total' =>$total ,
            ...
            $transport=$registerG*$result['gas']['Netcostes']->transport/100;
                            ...
                            if($FirstResidence==true){
                            $fixed_tax=$result['gas']['tax']->fixed_tax_first_res;

                            }elseif($FirstResidence==false){
                            $fixed_tax=$result['gas']['tax']->fixed_tax_not_first_res;
                            ...
                            $fixed_tax=$result['gas']['tax']->fixed_tax_first_res;
                                ...
                                $tax=(($result['gas']['tax']->energy_contribution+$result['gas']['tax']->federal_contribution+$result['gas']['tax']->contribution_protected_customers+$result['gas']['tax']->connection_fee)*$registerG)/100+$result['gas']['tax']->contribution_public_services;
                                $taxG=$tax*100;
                                ...
                                if($registerG>0){
                                $total=$totalGnet=$getProducts->ffG+(($getProducts->price_gasG*$registerG)/100);
                    ...
                    //  $tax=(($result['gas']['tax']->energy_contribution+$result['gas']['tax']->federal_contribution)*$registerG)/100+$result['gas']['tax']->contribution_public_services;
                    $tax=(($result['gas']['tax']->energy_contribution+$result['gas']['tax']->federal_contribution+$result['gas']['tax']->contribution_protected_customers+$result['gas']['tax']->connection_fee)*$registerG)/100+$result['gas']['tax']->contribution_public_services;
                    ...
                    $total=$totalGnet=$getProducts->ff+($getProducts->price_gas*($registerG/100));
                ...
                if($discount->discountcodeP){
                 ...
                 if($discount->discountType=='promo'){
                     ...
                     $promoD=$promoD+$disc;
                     if($discount->fuelType=='electricity'){
                         $promoED=$promoED+$disc;
                     }
                     if($discount->fuelType=='gas'){
                         $promoGD=$promoGD+$disc;
                     }
                     if($discount->fuelType=='all'){
                         $promoAD=$promoAD+$disc;
                     }
                 }
            ...
            if($sumRegisters==-1 && $registerG>=0){
               ...
               $totCunsum=$registerG;
                ...
                $totaldG=0; $promoD=0; $slD=0; $loyaltyD=0; 
                 $promoD=0; $promoED=0; $promoGD=0; $promoAD=0; 
...
$products['supplier']['signup_url'] = str_replace($replace, $info, $pack_url);
              ...
              if(count($products['price']['breakdown']['discounts'])>0){
                    $promo_discount = [
                        ...
                        'promo_discount' => $promoD,
                        'sl_discount' => $slD,
                        'loyalty_discount' => $loyaltyD,
                ...
                if($sumRegisters>=0 && $registerG==-1){
                   ...
                   // $inclPromo=$exclPromo-($totalDiscountE+$totalDiscountG);
                    ...
                    if(($totalEnet/100)-$promoED<0){
                        
                        $promoED=($totalEnet/100);
                    }
                    ...
                    'incl_promoD' => ($excl_promo/100)-$promoED-$promoGD-$promoAD,
                    'incl_slD' => ($excl_promo/100)-$slD,
                    'incl_loyaltyD' => ($excl_promo/100)-$loyaltyD,
                    
                    'incl_promoD_slD' => ($excl_promo/100)-$promoED-$promoGD-$promoAD-$slD,
                    'incl_promoD_loyaltyD' => ($excl_promo/100)-$promoED-$promoGD-$promoAD-$loyaltyD,
                    'incl_promoD_slD_loyaltyD' => ($excl_promo/100)-$promoED-$promoGD-$promoAD-$slD-$loyaltyD,
                    
                    'incl_slD_loyaltyD' => ($excl_promo/100)-$slD-$loyaltyD,
                    'test'=>$slD,
...
$incl_promo=$excl_promo-($totalDiscountAmount*100);
    ...
    if ($products['parameters']['values']['locale'] == 'nl') {
        ...
        if(isset($request->CurrentSupplierG)){
        ...
        if(isset($request->CurrentSupplierE)){
   ...
   if(isset($request->save_uuid)){
       
        if($request->save_uuid==1){
    ...
    if($saveuuid==1){
        ...
        $customerType='hot prospect';
        
        if(SearchDetail::where('uuid', $products['parameters']['uuid'] )->where('uuid','customer' )->exists()){ 
            
            $customerType='customer';
            
        }
        
        if(SearchDetail::where('uuid', $products['parameters']['uuid'] )->exists()){ 
            
            SearchDetail::where('uuid', $products['parameters']['uuid'] )->delete();
